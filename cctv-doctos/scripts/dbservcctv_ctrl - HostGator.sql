...
-- Procedimientos
--
CREATE DEFINER=`evosoluc`@`localhost` PROCEDURE `sp_01SelectClteReg` (IN `p_Tel` VARCHAR(25), IN `p_Email` VARCHAR(180), IN `p_Passw` VARCHAR(15), IN `p_Reg` INT)  BEGIN
...
CREATE DEFINER=`evosoluc`@`localhost` PROCEDURE `sp_01SelectCtrlPaqClte` (IN `p_ClteReg` VARCHAR(15))  BEGIN
	SELECT 
...
CREATE DEFINER=`evosoluc`@`localhost` PROCEDURE `sp_01SelectCtrlPaqCltesHist` (IN `p_ClteReg` VARCHAR(15))  BEGIN
	SELECT 
		a.IdClteReg, a.IdCtrlPaquete, a.IdPaquete, b.Paquete, a.UltimaFechaAct, a.FoliosOcupados, a.FoliosXOcupar, a.Vigente, 
        b.CantFolios, b.DiasVigencia
	FROM tblctrlpaquetes a
...
CREATE DEFINER=`evosoluc`@`localhost` PROCEDURE `sp_01SelectPaquetes` ()  BEGIN
	SELECT * FROM tblcat_paquetes;
END$$

CREATE DEFINER=`evosoluc`@`localhost` PROCEDURE `sp_02InsPaqClte` (IN `p_IdClteReg` VARCHAR(15), IN `p_IdPaquete` INT)  BEGIN
...
CREATE DEFINER=`evosoluc`@`localhost` PROCEDURE `sp_02InsRegistro` (IN `p_IdTipoClte` INT, IN `p_NombClte` VARCHAR(180), IN `p_NombNeg` VARCHAR(180), IN `p_Contacto` VARCHAR(180), IN `p_Tel` VARCHAR(25), IN `p_Email` VARCHAR(180), IN `p_Ciudad` VARCHAR(80), IN `p_Estado` VARCHAR(80), IN `p_Cve` VARCHAR(15))  BEGIN
...
CREATE DEFINER=`evosoluc`@`localhost` PROCEDURE `sp_03UpdClteReg` (IN `p_Email` VARCHAR(180), IN `p_Activo` INT)  BEGIN
	UPDATE tblregistros SET Activo = p_Activo WHERE Email = p_Email;
END$$

CREATE DEFINER=`evosoluc`@`localhost` PROCEDURE `sp_03UpdCodigoVerif` (IN `p_IdClteReg` VARCHAR(25))  BEGIN
...
CREATE DEFINER=`evosoluc`@`localhost` PROCEDURE `sp_03UpdRestauraCveClteReg` (IN `p_IdClteReg` VARCHAR(25), IN `p_CodigoVerif` INT, IN `p_Clave` VARCHAR(15))  BEGIN
